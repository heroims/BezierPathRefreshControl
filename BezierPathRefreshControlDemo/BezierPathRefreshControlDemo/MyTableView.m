...
{
    self = [super initWithFrame:frame plist:plist];
    if (self) {
...
- (void) addItemsOnTop
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
        ...
        sleep(3);
        [self performSelectorOnMainThread:@selector(finishRefreshControl) withObject:nil waitUntilDone:NO];
    });
...
{
    [self.slimeView finishingLoading];
}
